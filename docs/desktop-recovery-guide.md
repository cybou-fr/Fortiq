...
Implementation status: implemented for restoring a complete source folder from a selected snapshot. Updated September 2026.
...
The recovery executor does not require a running Fortiq service or a device-bound unlock key. On a new machine, the setup screen offers **Run as Portable**. Installed-mode GUI actions may ask you to reopen Fortiq as administrator through Windows UAC; repeat the action in the new instance.

## Restore a backup

1. Open **Recovery**, then **Restore files from a recovery kit**. This action is available even when the local list of protected sources is empty.
...
6. Choose an existing destination parent folder. Review the full new recovery-folder path shown below it.
7. Select **Restore all files from selected backup**. Wait for the completion message, then use **Open restored folder**.

Fortiq restores the selected snapshot's complete source subtree. It does not offer individual-file selection yet. It does not overwrite existing destination folders or restore inside the source, repository or recovery-kit directory. Destination ancestors containing symbolic links or junctions are rejected. The engine restores through a private staging directory and checks the restored tree before publishing the destination.
