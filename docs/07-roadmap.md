...
- [ ] **In-App Component & Third-Party Engine Updater**: Transactional, rollback-safe self-updates for desktop, background service, and pinned restic engines under TUF with project-held offline root keys ([ADR-008 Revision 1](adr/ADR-008-update-trust.md), ADR-014). Authenticode signatures are verified when present and recorded when absent; no certificate purchase gates this milestone. *Engine done: `TufTrustedMetadata` refuses rollback, freeze, mix-and-match and root-rotation attacks; `UpdateTransaction` stages, swaps and rolls back crash-safely; `ComponentUpdater` joins them and `Fortiq.Service` recovers an interrupted update at start-up. Delivery (an `IUpdateContentSource` that fetches, and a release that publishes TUF metadata) and the desktop updates manager remain.*
...
- [x] **Audit Receipts & History Inspector**: the Backups screen's History tab reads the receipts themselves (`ReceiptTimeline`) rather than restating the dashboard's three timestamps — every operation in order, failures with their reasons, and receipts predating the chained schema shown as unverifiable. The ledger verdict `AuditLedgerVerifier` computes is displayed on the same screen (Spec 17 section 7);
...
- [x] **Operation without administrator rights**: the local `Fortiq Operators` group named in ADR-007's
  discussion now exists (`FortiqOperatorsGroup`), created empty by the installer, and
  `ServiceIpcAuthorization` authorises per command at two levels — provisioning stays with
  administrators, everyday operations are delegated;
