...
- **Cryptographic Audit Ledger & Evidence**: `FileSystemOperationReceiptStore` writes structured operation receipts (`fortiq.operation-receipt`, schema v2) cryptographically chained via SHA-256 digests (`PreviousReceiptHash` and `ReceiptHash`) and reconciled with a repository-scoped ledger (`.ledger`). `AuditLedgerVerifier` enforces ledger integrity and chain continuity, ensuring `HealthPublisher` trusts only verified evidence and flags tampering as `audit-ledger-tampered` (`HealthVerdict.AtRisk`). External anchoring of ledger heads is implemented as a capability (`IAuditLedgerAnchor`, with file and Windows ETW implementations) but **is not yet wired into production operations**: scheduled backups, retention and restore proofs still construct their receipt store without an anchor, so no head is currently anchored outside the receipt directory.
